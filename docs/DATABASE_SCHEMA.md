...
CREATE INDEX idx_sensor_metric ON sensor_readings(metric_name, time DESC);
...
CREATE INDEX idx_traffic_intersection ON traffic_flows(intersection_id, time DESC);
...
CREATE TABLE roads (
  ...
  surface VARCHAR(30),
  source BIGINT,  -- pgRouting vertex
  target BIGINT,  -- pgRouting vertex
  cost FLOAT,     -- travel time seconds
  reverse_cost FLOAT
);
