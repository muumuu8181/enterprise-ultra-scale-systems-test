...
#### transaction_headers
取引の論理的なヘッダー情報（トランザクションID、種別、ステータス、冪等性キー）。
```sql
...
全ての入出金取引明細を記録する。**月次パーティション対象**。
```sql
CREATE TABLE transactions (
    entry_id UUID NOT NULL, -- PKの一部
    transaction_id UUID REFERENCES transaction_headers(transaction_id),
    account_id UUID NOT NULL REFERENCES accounts(account_id),
    amount DECIMAL(19,4) NOT NULL,
    direction VARCHAR(10) NOT NULL, -- DEBIT, CREDIT
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    PRIMARY KEY (entry_id, created_at) -- パーティションキーを含める
