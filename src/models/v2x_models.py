...
class EmergencyVehicle(Base):
    ...
    Attributes:
        id (int): 内部ID
        vehicle_id (str): 車両識別子
        type (str): 車両タイプ (ambulance, fire, police)
...
class Intersection(Base):
    ...
    Attributes:
        id (int): 交差点ID
        name (str): 交差点名
        location (Geometry): 位置 (POINT)
        current_phase (str): 現在の信号フェーズ
    """
    ...
    location: Mapped[Any] = mapped_column(Geometry("POINT", srid=4326))
    current_phase: Mapped[str] = mapped_column(String, default="RED")


class PreemptionRequest(Base):
    """
    信号優先制御要求モデル
    ...
    Attributes:
        ...
        route (dict): 経路情報 (JSON)
        status (str): ステータス (pending, granted, completed, rejected)
        created_at (datetime): 作成日時
        updated_at (datetime): 更新日時
    """
    __tablename__ = "preemption_requests"
